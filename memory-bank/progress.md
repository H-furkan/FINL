...
## Lessons Learned
- Sibling doc linking fixed multi-hop failures from vanilla TF-IDF
- drug_knowledge.csv gives cleaner LLM answers via enriched context
- Fallback mode (no LLM) achieves 74% -- strong baseline
- Comparison/safety categories are weakest -- broad multi-drug queries need wider retrieval
- Free model availability on OpenRouter changes frequently -- keep model list up to date
- API keys pushed to public GitHub repos get auto-revoked by providers -- use secrets
